...
export const whatsappConsents = sqliteTable(
  "whatsapp_consent",
  (d) => ({
    conversationSid: d.text({ length: 255 }).notNull().primaryKey(),
    toNumber: d.text({ length: 50 }).notNull(),
    apt: d.text({ length: 100 }).notNull(),
    visitor: d.text({ length: 255 }).notNull(),
    company: d.text({ length: 255 }).notNull(),
    status: d
      .text({ length: 20 })
      .notNull()
      .default("pending")
      .$type<"pending" | "approved" | "denied" | "no_answer" | "failed">(),
    lastMsgSid: d.text({ length: 255 }),
    decidedAt: d.integer({ mode: "timestamp" }),
    transcript: d.text(), // JSON stored as text
    ttlSeconds: d.integer({ mode: "number" }).notNull().default(300),
    createdAt: d
      .integer({ mode: "timestamp" })
      .default(sql`(unixepoch())`)
      .notNull(),
